...
import { toggleFreedomTask, deleteFreedomTask, createFreedomTask, updateFreedomTaskDescription, updateFreedomTaskCategory, type FreedomTask } from "@/lib/actions/freedom";
...
function InlineAddTask({ category, onAdd }: { category: FreedomCategory | null; onAdd: (t: FreedomTask) => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      await createFreedomTask({ title: title.trim(), priority, due_date: dueDate || undefined, category: category ?? undefined });
    ...
    <>
      <tr className={cn("group border-b border-gray-100 hover:bg-gray-50 transition-colors", task.completed && "opacity-50")}>
        ...
        <td className="py-2.5 px-4 text-xs whitespace-nowrap">
          {task.due_date
            ? <span className={cn("flex items-center gap-1", isOverdue ? "text-red-500 font-medium" : "text-gray-400")}><Calendar size={11} />{formatDate(task.due_date)}</span>
        ...
        <td className="py-2.5 px-4">
          <select value={task.category ?? ""}
            ...
            className={cn(
              ...
              task.category ? FREEDOM_CATEGORY_STYLES[task.category].pill : "text-gray-400 bg-gray-100"
            )}>
            <option value="">Uncategorized</option>
            {FREEDOM_CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}
          </select>
        ...
        <tr className="border-b border-gray-100 bg-gray-50">
          <td colSpan={6} className="px-10 py-2">
            <div className="space-y-2">
              <textarea autoFocus value={draft} onChange={(e) => setDraft(e.target.value)} onBlur={handleDescBlur}
                placeholder="Add a description…" rows={2}
...
function AddTableRow({ onAdd, onCancel }: { onAdd: (t: FreedomTask) => void; onCancel: () => void }) {
  const [title, setTitle] = useState("");
  const [priority, setPriority] = useState<"low" | "medium" | "high">("medium");
  const [dueDate, setDueDate] = useState("");
  const [category, setCategory] = useState<FreedomCategory | "">("");
  const [loading, setLoading] = useState(false);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    onAdd({
      id: Date.now(), title: title.trim(), description: null,
      completed: false, priority, due_date: dueDate || null,
      category: (category || null) as FreedomTask["category"],
  ...
  return (
    <tr className="border-b border-indigo-100 bg-indigo-50">
      <td className="py-2 px-4">
        <form id="add-freedom-row" onSubmit={handleSubmit}>
          <input autoFocus value={title} onChange={(e) => setTitle(e.target.value)}
      ...
      <td className="py-2 px-4">
        <select value={category} onChange={(e) => setCategory(e.target.value as FreedomCategory | "")}
          className="text-xs bg-white border border-gray-200 rounded-lg px-2 py-1.5 focus:outline-none w-full">
      ...
      <td className="py-2 px-4 text-center">
        <div className="flex items-center justify-center gap-2">
          <button type="submit" form="add-freedom-row" disabled={loading || !title.trim()}
            className="text-xs bg-indigo-600 text-white px-3 py-1 rounded-lg hover:bg-indigo-700 disabled:opacity-50">
            {loading ? "…" : "Add"}
          </button>
...
function CalendarView({ tasks, onToggle }: { tasks: FreedomTask[]; onToggle: (id: number) => void }) {
  ...
  const tasksByDate: Record<string, FreedomTask[]> = {};
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
      ...
      <div className="grid grid-cols-7 border-b border-gray-100">
        {DAY_NAMES.map((d) => <div key={d} className="text-center py-2 text-xs font-semibold text-gray-400">{d}</div>)}
      </div>
        ...
        {cells.map((day, i) => {
          ...
          return (
            <div key={dateStr} className={cn("min-h-[90px] p-1.5 flex flex-col", isPast && !isToday && "bg-gray-50/40")}>
              <span className={cn("text-xs font-semibold w-6 h-6 flex items-center justify-center rounded-full mb-1 self-start",
                isToday ? "bg-indigo-600 text-white" : isPast ? "text-gray-300" : "text-gray-600")}>{day}</span>
              <div className="flex flex-col gap-0.5 flex-1">
                {dayTasks.slice(0, 3).map((t) => {
                  const cat = t.category ? FREEDOM_CATEGORY_STYLES[t.category] : null;
                  return (
                    <button key={t.id} onClick={() => onToggle(t.id)} title={t.title}
                      className={cn("text-left text-[10px] px-1.5 py-0.5 rounded truncate w-full leading-snug",
                        t.completed ? "opacity-40 line-through" : "", cat ? cat.pill : "bg-indigo-100 text-indigo-700")}>
                      {t.title}
                    </button>
                  );
                })}
      ...
      {undated.length > 0 && (
        <div className="border-t border-gray-100 px-4 py-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {undated.map((t) => {
              const cat = t.category ? FREEDOM_CATEGORY_STYLES[t.category] : null;
              return <span key={t.id} className={cn("text-xs px-2 py-0.5 rounded-full font-medium", cat ? cat.pill : "bg-gray-100 text-gray-500")}>{t.title}</span>;
            })}
          </div>
        </div>
      )}
...
export default function FreedomTaskList({ initialTasks }: { initialTasks: FreedomTask[] }) {
  ...
  function handleCategoryChange(id: number, category: FreedomCategory | null) {
    setTasks((prev) => prev.map((t) => t.id === id ? { ...t, category } : t));
    startTransition(() => updateFreedomTaskCategory(id, category));
  }
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex gap-0.5 bg-gray-100 p-0.5 rounded-lg">
            <button onClick={() => setTopView("upcoming")}
            ...
            <button onClick={() => setTopView("calendar")}
              className={cn("flex items-center gap-1.5 px-3 py-1 rounded-md text-xs font-medium transition-colors",
        ...
        {topView === "upcoming" ? (
          <>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {FREEDOM_CATEGORIES.map((cat) => (
                <CategoryColumn key={cat} category={cat}
                  tasks={tasks.filter((t) => t.category === cat)}
                  onToggle={handleToggle} onDelete={handleDelete} onAdd={handleAdd}
                  onDescriptionSave={handleDescriptionSave} onCategoryChange={handleCategoryChange} />
              ))}
            </div>
            {hasUncategorized && (
          ...
          <table className="w-full">
            ...
            <tbody>
              ...
              {tableTasks.length === 0 && !showAddRow ? (
                ...
                tableTasks.map((t) => (
                  <TableRow key={t.id} task={t} onToggle={handleToggle} onDelete={handleDelete}
                    onDescriptionSave={handleDescriptionSave} onCategoryChange={handleCategoryChange} />
                ))
              )}
            </tbody>
